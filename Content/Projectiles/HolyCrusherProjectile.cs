...
namespace Ultraconyx.Content.Projectiles;
	...
	public class HolyCrusherProjectile : ModProjectile
	{
		...
		public override void AI() 
		{
			...
			{
				Color dustColor = Color.Lerp(Color.HotPink, Color.White, Main.rand.NextFloat() * 0.3f);
			...
			{
				Dust.NewDustPerfect(
					Projectile.Center + dustRotation.ToRotationVector2() * 84f * Projectile.scale, 
					DustID.TintableDustLighted, 
					dustVelocity, 
					100, 
					Color.LightPink * Projectile.Opacity, 
		...
		public override bool PreDraw(ref Color lightColor) 
		{
			...
			faintLightingColor.R = (byte)(faintLightingColor.R * lightingColor);
			faintLightingColor.G = (byte)(faintLightingColor.G * (0.25f + lightingColor * 0.75f));
			faintLightingColor.B = (byte)(faintLightingColor.B * (0.25f + lightingColor * 0.75f));

			Main.EntitySpriteDraw(texture, position, sourceRectangle, backDarkColor * lightingColor * lerpTime, 
				Projectile.rotation + Projectile.ai[0] * MathHelper.PiOver4 * -1f * (1f - percentageOfLife), origin, scale, spriteEffects, 0f);
			...
			Main.EntitySpriteDraw(texture, position, sourceRectangle, middleMediumColor * lightingColor * lerpTime * 0.3f, 
				Projectile.rotation, origin, scale, spriteEffects, 0f);
			
			Main.EntitySpriteDraw(texture, position, sourceRectangle, frontLightColor * lightingColor * lerpTime * 0.5f, 
				Projectile.rotation, origin, scale * 0.975f, spriteEffects, 0f);
			...
			for (float i = 0f; i < 8f; i += 1f) 
			{
				...
				DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos, 
					new Color(255, 200, 255, 0) * lerpTime * (i / 9f), 
					middleMediumColor, percentageOfLife, 0f, 0.5f, 0.5f, 1f, edgeRotation, 
					new Vector2(0f, Utils.Remap(percentageOfLife, 0f, 1f, 3f, 0f)) * scale, Vector2.One * scale);
			}

			Vector2 drawPos2 = position + (Projectile.rotation + Utils.Remap(percentageOfLife, 0f, 1f, 0f, MathHelper.PiOver4) * Projectile.ai[0]).ToRotationVector2() * 
				((float)texture.Width * 0.5f - 4f) * scale;
			DrawPrettyStarSparkle(Projectile.Opacity, SpriteEffects.None, drawPos2, 
				new Color(255, 200, 255, 0) * lerpTime * 0.5f, 
				middleMediumColor, percentageOfLife, 0f, 0.5f, 0.5f, 1f, 0f, 
				new Vector2(2f, Utils.Remap(percentageOfLife, 0f, 1f, 4f, 1f)) * scale, Vector2.One * scale);

			return false;
		}

		private static void DrawPrettyStarSparkle(float opacity, SpriteEffects dir, Vector2 drawPos, Color drawColor, 
			Color shineColor, float flareCounter, float fadeInStart, float fadeInEnd, float fadeOutStart, float fadeOutEnd, 
