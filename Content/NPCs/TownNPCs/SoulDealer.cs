...
[AutoloadHead]
public class SoulDealer : ModNPC
{
    private const int AuraRadius = 150;
    ...
    public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
    {
        bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement>
        {
            ...
            new FlavorTextBestiaryInfoElement("A mysterious dealer who trades in souls. He can enchant weapons with dark power for a price.")
        });
    }
    ...
    public override string GetChat()
    {
        ...
        List<string> chat = new List<string>
        {
            ...
            "The Mechanical Horrors have some interesting souls. Expensive, but worth it.",
            "Souls are like currency to me. Each has its own... flavor.",
            "The corruption grows stronger every day. Good for business.",
            "You've faced the Wall of Flesh? Impressive. That opens up new opportunities.",
    ...
    public override void OnChatButtonClicked(bool firstButton, ref string shop)
    {
        ...
        {
            // Open enchantment UI
            SoulDealerUI.ShowEnchantUI = true;
    ...
    private NPC FindClosestEnemy()
    {
        ...
        for (int i = 0; i < Main.maxNPCs; i++)
        {
            ...
            {
                if (auraRect.Intersects(target.getRect()))
                {
    ...
    private void CreateCircularAura()
    {
        ...
        for (int i = 0; i < particleCount; i++)
        {
            ...
            if (Main.rand.NextBool(2))
            {
                ...
                Vector2 trailPos = NPC.Center + new Vector2(
    ...
    {
        return new SoulDealerProfile();
    }
}

public class SoulDealerProfile : ITownNPCProfile
{
...
public class SoulDealerUI : ModSystem
{
    ...
    public override void Load()
    {
        if (!Main.dedServ)
        {
            enchantInterface = new UserInterface();
            enchantUI = new EnchantUI();
            enchantUI.Activate();
    ...
    public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers)
    {
        ...
        if (mouseTextIndex != -1)
        {
            layers.Insert(mouseTextIndex, new LegacyGameInterfaceLayer(
                "SoulDealer: Enchant UI",
                delegate
                {
                    if (ShowEnchantUI && enchantInterface?.CurrentState != null)
                    {
    ...
    public static void ToggleEnchantUI()
    {
        ...
        else
        {
            ...
            // Clear mouse interface when closing
...
public class EnchantUI : UIState
{
    private UIPanel mainPanel;
    private ReforgeItemSlot itemSlot;
    ...
    public override void OnInitialize()
    {
        ...
        enchantButton.Height.Set(40f, 0f);
        enchantButton.HAlign = 0.5f;
        enchantButton.Top.Set(150f, 0f);
        enchantButton.OnClick += EnchantButtonClicked;
        enchantButton.BackgroundColor = new Color(128, 0, 128);
        ...
        closeButton.Top.Set(-15f, 0f);
        closeButton.OnClick += CloseButtonClicked;
        closeButton.BackgroundColor = new Color(200, 0, 0);
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (isDragging)
        {
            mainPanel.Left.Set(Main.mouseX - dragOffset.X, 0f);
    ...
    private void UpdatePriceDisplay()
    {
        if (enchantItem != null && !enchantItem.IsAir)
        {
            ...
            if (isWeapon)
            {
                ...
                {
                    priceText.SetText("Already enchanted!");
                    priceText.TextColor = Color.Purple;
            ...
            {
                priceText.SetText("Not a valid weapon");
                priceText.TextColor = Color.Red;
                enchantButton.BackgroundColor = new Color(100, 100, 100);
            }
        }
        else
        {
            priceText.SetText("Place a weapon here");
            priceText.TextColor = Color.LightGray;
            enchantButton.BackgroundColor = new Color(100, 100, 100);
        }
    }
    ...
    private int CalculatePrice(Item item)
    {
        ...
        int damageBonus = (int)(item.damage * 100); // 100 copper per damage
    ...
    private void EnchantItem(Item item)
    {
        ...
        else if (item.DamageType.CountsAsClass(DamageClass.Ranged))
        {
            ...
            if (item.useTime > 5) item.useTime = (int)(item.useTime * 0.95f);
        }
        else if (item.DamageType.CountsAsClass(DamageClass.Magic))
        {
            item.mana = (int)(item.mana * 0.9f);
            item.crit += 5;
        }
...
// REFORGE ITEM SLOT - SIMPLE VERSION that WORKS
public class ReforgeItemSlot : UIElement
{
    ...
    public override void LeftClick(UIMouseEvent evt)
    {
        ...
        // If slot has an item and player's mouse is empty, take it out
        else if (!StoredItem.IsAir && Main.mouseItem.IsAir)
        {
            Main.mouseItem = StoredItem.Clone();
            StoredItem.TurnToAir();
            SoundEngine.PlaySound(SoundID.Grab);
        }
        // If both have items, swap them (only if mouse item is a weapon)
        else if (!StoredItem.IsAir && !Main.mouseItem.IsAir)
        {
            if (IsWeapon(Main.mouseItem))
            {
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch)
    {
        ...
        if (!StoredItem.IsAir)
        {
            ...
            {
                string text = StoredItem.stack.ToString();
                Vector2 textSize = FontAssets.ItemStack.Value.MeasureString(text);
                Vector2 textPos = dimensions.Position() + new Vector2(rect.Width - textSize.X - 2f, rect.Height - textSize.Y - 2f);
...
{
    private string text;
    public Color BackgroundColor { get; set; } = new Color(63, 82, 151);
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch)
    {
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            Vector2 textPos = dimensions.Center() - textSize / 2f;
            Utils.DrawBorderString(spriteBatch, text, textPos, TextColor);
        }
    }
}

// Global item class to track enchantments
public class EnchantedItemGlobal : GlobalItem
{
    ...
    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
    {
        if (IsEnchanted)
        {
            // Find the name tooltip and change its color to purple
            ...
            if (item.DamageType.CountsAsClass(DamageClass.Melee))
                ...
                tooltips.Add(new TooltipLine(Mod, "EnchantBonusRanged", "+10% velocity, -5% use time"));
            else if (item.DamageType.CountsAsClass(DamageClass.Magic))
                tooltips.Add(new TooltipLine(Mod, "EnchantBonusMagic", "-10% mana cost, +5% critical strike chance"));
            else if (item.DamageType.CountsAsClass(DamageClass.Summon))
